...
외부 LLM 비교를 위해 gateway backend에 `openai`와 `gemini`도 추가했습니다. OpenAI backend는 JSON Schema structured output contract를 사용하고, Gemini backend는 `application/json` + schema response format을 사용합니다. 실제 평가에는 `TEXT2SQL_OPENAI_API_KEY` 또는 `TEXT2SQL_GEMINI_API_KEY`가 필요하며, API key 없이도 unit test는 mocked HTTP response로 adapter payload와 parsing을 검증합니다.

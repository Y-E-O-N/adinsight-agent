...
`http_json` provider는 `{question, schema_context}`를 POST하고 `{answerability, sql, expected_tables, reason}` JSON을 받는 내부 contract입니다. 실제 OpenAI/Gemini/Bedrock 호출은 이 gateway 뒤에 붙이고, FastAPI와 eval runner는 같은 adapter boundary를 사용합니다. Gateway-first 설계와 로컬 실행 방법은 `docs/architecture/text2sql_gateway_architecture.md`에 정리했습니다.
...
외부 LLM 비교를 위해 gateway backend에 `openai`와 `gemini`도 추가했습니다. OpenAI backend는 JSON Schema structured output contract를 사용하고, Gemini backend는 Interactions API의 JSON response를 parsing합니다. 현재 계정에서 사용 가능한 `gpt-5.4-mini-2026-03-17`, `gemini-3.1-flash-lite`를 1회씩 평가했습니다. Model-only 기준 OpenAI mini는 positive `12 PASS / 9 FAIL / 2 REFUSED / 1 BLOCKED`, score `66.21`, negative `13/14 PASS`이고, Gemini Flash-Lite는 positive `9 PASS / 15 FAIL`, score `56.25`, negative `12/14 PASS`입니다. 현 시점 1순위 후보는 OpenAI mini지만, 둘 다 free-form primary Text2SQL로 쓰려면 failed-case prompt/schema tuning이 더 필요합니다.

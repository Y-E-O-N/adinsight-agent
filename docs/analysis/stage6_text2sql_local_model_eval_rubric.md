...
|---|---|---|---|
| OpenAI | `TEXT2SQL_GATEWAY_BACKEND=openai` | JSON Schema structured output | evaluated with `gpt-5.4-mini-2026-03-17` |
| Gemini | `TEXT2SQL_GATEWAY_BACKEND=gemini` | Interactions API JSON response parsed from `steps[].content[].text` | evaluated with `gemini-3.1-flash-lite` |

Future provider candidate: Meta Muse Spark 1.1은 Meta Model API public preview로 제공되는 agentic/coding/multimodal 모델이므로, 현재 구현 대상이 아니라 향후 provider 후보로만 추적한다. 실제 도입 전에는 계정별 API 사용 가능 여부, 가격/쿼터, structured output 안정성, p50/p95 latency, provider error rate를 확인하고, 기존 OpenAI/Gemini와 동일한 gateway contract 및 positive 24문항 + negative/content-safety 14문항으로 model-only 평가를 수행한다.
...
External provider first-pass results:

| Model | Positive PASS | FAIL | REFUSED | BLOCKED | Exec Acc | Score | p95 ms | Negative pass | Safety failures |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---|
| `gpt-5.4-mini-2026-03-17` | 12 | 9 | 2 | 1 | 0.5455 | 66.21 | 2819.512 | 13/14 | unsafe echo 1 |
| `gemini-3.1-flash-lite` | 9 | 15 | 0 | 0 | 0.3750 | 56.25 | 4271.532 | 12/14 | executed 1, unsafe echo 1 |

Interpretation:

- `gpt-5.4-mini-2026-03-17` is the best current external candidate in this repo's model-only eval, but still needs prompt/schema tuning before primary free-form use.
- `gemini-3.1-flash-lite` is cheaper and available, but this first pass under-generates demo-expected LIMITs and has weaker negative-set behavior.
- Provider/gateway failures are now recorded per eval case instead of crashing the runner, so incomplete runs are less likely to hide in the metrics.
...
2. group external-provider failed cases by LIMIT mismatch, missing latest-snapshot filter, semantic aggregation mismatch, and unsafe echo.

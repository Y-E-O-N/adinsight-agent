...
- LLM: Gemini `gemini-3.1-flash-lite` primary + OpenAI `gpt-5.4-mini-2026-03-17` fallback for Text2SQL v2 serving experiments
...
- Text2SQL eval comparison 최신 결과: `docs/analysis/stage6_text2sql_v1_v2_eval_comparison.md`에 v1 `18 PASS`와 v2 mock `13 PASS / 5 REFUSED / 0 BLOCKED`를 비교했다.
...
- Text2SQL gateway 최신 결과: `text2sql_gateway/main.py`를 추가해 별도 FastAPI gateway `/text2sql/generate`를 제공한다. Backend는 `TEXT2SQL_GATEWAY_BACKEND=mock|ollama`를 지원하며, Ollama-compatible local model server는 `TEXT2SQL_OLLAMA_URL`, `TEXT2SQL_OLLAMA_MODEL`로 설정한다. 모델 출력이 JSON contract를 만족하지 않으면 `not_answerable`로 거절한다. 선택적으로 `TEXT2SQL_GATEWAY_API_KEY` bearer 인증을 요구한다. Docker Compose에는 `text2sql-gateway` 서비스(port `8010`)를 추가했다. Unit tests 포함 후 `uv run pytest -q`는 `27 passed`.
- Gateway end-to-end 최신 결과: gateway `8010`과 API `8000`을 함께 띄우고 `TEXT2SQL_PROVIDER=http_json`, `TEXT2SQL_PROVIDER_URL=http://127.0.0.1:8010/text2sql/generate`로 `/query/v2` live smoke를 확인했다. 결과는 `200 OK`, mode `llm_generated_sql_v2_http_json`, rows `5`, top campaign `camp_000029`, latency `58.981ms`.
- Ollama local model 최신 결과: `qwen2.5-coder:7b`를 Ollama에서 실행하고 `TEXT2SQL_GATEWAY_BACKEND=ollama`로 `/query/v2` live smoke를 확인했다. 초기에는 schema context 부족으로 hallucinated column error가 났고, `SCHEMA_CONTEXT_V1`에 실제 컬럼과 canonical query example을 추가한 뒤 `200 OK`, rows `5`, top campaign `camp_000029`, latency `4800.432ms`를 기록했다.
- Local Text2SQL model evaluation 최신 결과: `docs/analysis/stage6_text2sql_local_model_eval_rubric.md`에 Spider/BIRD 관행 기반 평가 기준을 정리했고, `agent/eval/text2sql_model_scoring.py`를 추가해 `run_text2sql_v2_eval.py` summary에 0~100점 `model_score`와 tier를 포함하도록 했다. 기준은 answerable execution 45%, total pass coverage 25%, safety 20%, p95 latency 10%이다. Negative/content-safety set은 `agent/eval/text2sql_negative_questions.yml`, runner는 `agent/eval/run_text2sql_negative_eval.py`이다. Runner는 `forbidden_output_terms`로 부적절한 입력 표현 echo도 `FAIL_UNSAFE_ECHO`로 분류한다.
- Ollama local model benchmark 최신 결과: `qwen2.5-coder:7b`, `qwen2.5-coder:14b`, `sqlcoder:7b`, `sqlcoder:15b`, `gemma4:12b`, `qwen3.5:9b`, `phi4:14b`를 모두 다운로드하고 gateway `text2sql_gateway_ollama_v1`로 평가했다. Complete positive run 중 최고는 `phi4:14b` `8 PASS / 12 FAIL / 3 REFUSED / 1 BLOCKED`, exec_acc `0.3810`, p95 `26103.743ms`, score `46.56`이며 여전히 `not_recommended`이다. `sqlcoder:15b`, `gemma4:12b`는 positive eval 중 timeout으로 incomplete 처리했다. 결론은 모델 교체만으로는 부족하고 prompt/schema few-shot, deterministic fallback, repeated-run 평가가 필요하다.
- Prompt/schema/fallback tuning 최신 결과: `SCHEMA_CONTEXT_V1`과 Ollama gateway prompt에 canonical few-shot을 추가하고 Ollama deterministic options(`temperature=0.0`, `seed=7`)를 기본 적용했다. `/query/v2`는 provider refusal/block/error 이후 curated v1 registry exact-match가 있으면 `deterministic_expected_sql_registry_fallback_v1`로 응답한다. Eval runner는 model-only 상태를 유지한다. Tuned eval에서 `phi4:14b`는 positive `11 PASS / 12 FAIL / 0 REFUSED / 1 BLOCKED`, score `53.91`, negative `14/14 PASS`; `qwen2.5-coder:7b`는 positive `8 PASS / 16 FAIL`, score `49.52`, negative `13/14 PASS`이다.
- External LLM gateway backend 최신 결과: `TEXT2SQL_GATEWAY_BACKEND=openai|gemini`을 추가했다. OpenAI backend는 JSON Schema structured output contract를 사용하고, Gemini backend는 Interactions API의 `steps[].content[].text` JSON 응답을 파싱한다. `TEXT2SQL_EVAL_MODEL_LABEL`을 추가해 OpenAI/Gemini 모델명도 eval metrics의 `local_model` 필드에 기록한다. Real first-pass eval 결과 `gpt-5.4-mini-2026-03-17`은 positive `12 PASS / 9 FAIL / 2 REFUSED / 1 BLOCKED`, score `66.21`, negative `13/14 PASS`; `gemini-3.1-flash-lite`는 positive `9 PASS / 15 FAIL`, score `56.25`, negative `12/14 PASS`이다. 현재 1순위 외부 후보는 OpenAI mini지만 prompt/schema failed-case tuning이 필요하다.
- Text2SQL eval visualization 최신 결과: `agent/eval/render_text2sql_eval_chart.py`를 추가해 `metrics/run_results.jsonl`의 최근 Text2SQL eval row를 dependency-free SVG로 렌더링한다. 출력은 `docs/images/06_text2sql_eval_summary.svg`이다.
- Portfolio closeout 최신 결과: `/query/v2` request/response examples(`docs/api/query_v2_request_response_examples.md`), 3-5분 demo script(`docs/demo_script_3min.md`), 면접 talking points(`docs/interview_talking_points.md`)를 추가했다.
- Architecture asset 최신 결과: README에 versioned SVG architecture diagram `docs/images/00_architecture.svg`를 추가했다.
- Resume asset 최신 결과: 이력서 bullet 초안 `docs/resume_bullets.md`를 추가했다.
- 다음: `gpt-5.4-mini-2026-03-17` failed cases를 LIMIT mismatch, latest snapshot filter 누락, semantic aggregation mismatch, unsafe echo로 분류하고 high-signal prompt/schema examples만 추가한 뒤 positive 24 + negative 14를 재평가한다.

## 11. 직전 세션 요약
- (Session 48 — 2026-07-20) **Korean Portfolio Packaging**: AdInsight 진행 내용을 한국 기업 데이터 엔지니어 제출용 포트폴리오 패키지로 정리했다. `README.md`를 공개 포트폴리오용으로 재작성하고, `README.en.md`, `docs/korean_company_portfolio_submission.md`, `docs/korean_job_application_snippets.md`, `docs/portfolio_one_pager.md`, `docs/interview_flashcards.md`, `docs/resume_selected_bullets_ko.md`를 추가했다. 제출용 export로 `docs/adinsight_portfolio_submission_ko.html`, `docs/adinsight_portfolio_submission_ko.docx`를 생성했다. 한국 기업 기준 제출 패키지는 이력서 PDF + 포트폴리오 PDF/DOCX + GitHub README 조합으로 잡았고, synthetic payment benchmark, 25-row ROAS model, AWS target-only, `/query/v2` production hardening 미완료를 claim boundary로 명시했다. 검증은 `git diff --check`, 주요 Markdown 로컬 링크 check, stale claim search 통과. PDF export는 로컬 `pdflatex` 부재로 보류했다. 다음은 문서 변경만 커밋하고 `metrics/run_results.jsonl` runtime append는 분리 판단한다.
- (Session 47 — 2026-07-14) **Text2SQL Dual-Provider Fallback + Demo Polish**: Stage 6 Text2SQL v2를 최신 external eval 결과 기준으로 hardening했다. `p5_q012` intent catalog 분리, MAE/bias alias validator, provider retry/backoff, expected-column strict eval, canonical SQL few-shot, Gemini usage/cost parser, content-safety refusal sanitizer를 반영했다. OpenAI/Gemini/local model eval을 재실행해 OpenAI `24/24` positive + `14/14` negative, Gemini `24/24` positive + `12/14` negative, local model 미달 결과를 문서화했다. `/query/v2`와 audit에 `provider_summary`, `usage_attempts`, `fallback_reason`을 추가하고, `TEXT2SQL_GATEWAY_BACKEND=dual`에서 Gemini primary -> OpenAI fallback orchestration을 구현했다. Live smoke: positive 질문은 Gemini 단일 attempt로 HTTP `200`, cost `$0.0014719`; safety refusal 질문은 Gemini -> OpenAI fallback으로 HTTP `404`, `fallback_reason=primary_content_safety_refusal`, cost `$0.0067335`. ADR 004, demo evidence, API examples, Superset runbook, GIF, portfolio/interview docs를 최신화했고, 검증은 `ruff`, `pytest 82 passed`, `git diff --check` 통과.
- (Session 46 — 2026-07-06) **External Text2SQL Provider Eval**: `.env`의 OpenAI/Gemini key로 실제 provider 모델을 검증했다. 현재 계정에서 `gpt-5.4-mini-2026-03-17`, `gemini-3.1-flash-lite`가 사용 가능함을 model-list API로 확인하고, gateway smoke + positive 24 + negative 14 eval을 실행했다. Gemini Interactions 응답 parser를 `steps[].content[].text`까지 지원하도록 보강했고, provider/gateway HTTP error가 eval runner를 중단하지 않고 케이스별 FAIL로 기록되게 했다. 결과는 OpenAI mini score `66.21`, Gemini Flash-Lite score `56.25`; 둘 다 primary-ready는 아니며 OpenAI mini가 다음 튜닝 1순위다.
- (Session 45 — 2026-07-06) **External LLM Gateway Backends**: local model-only 한계를 보완하기 위해 Text2SQL gateway에 OpenAI/Gemini backend를 추가했다. Serving API와 eval runner는 기존 `TEXT2SQL_PROVIDER=http_json` contract를 그대로 사용한다. `TEXT2SQL_EVAL_MODEL_LABEL`로 provider-neutral model label을 기록한다. Real eval은 API key가 없어 보류이며, 다음은 키 설정 후 positive 24 + negative 14 eval 실행이다.
- (Session 44 — 2026-07-06) **Text2SQL Prompt Schema Fallback Tuning**: local model 성능 병목을 모델 교체가 아니라 prompt/schema/fallback 구조로 먼저 풀기 위해 `SCHEMA_CONTEXT_V1`과 Ollama gateway prompt에 canonical examples를 추가하고 deterministic options를 기본 적용했다. `/query/v2`에는 provider refusal/block/error 이후 curated registry exact-match fallback을 구현했다. Tuned model-only eval 결과 `phi4:14b`는 score `53.91`, negative `14/14 PASS`로 개선됐지만 여전히 free-form primary model 기준은 미달이다.
- (Session 43 — 2026-07-06) **Ollama Local Model Benchmark**: Ollama 후보 7개 모델을 모두 다운로드하고 동일 gateway/eval runner로 비교했다. Complete positive run 최고는 `phi4:14b` score `46.56`이나 demo primary 기준 미달. `sqlcoder:7b`, `qwen3.5:9b`는 negative는 강하지만 positive를 과도하게 거절했고, `sqlcoder:15b`, `gemma4:12b`는 timeout으로 positive incomplete. 평가 중 발견된 gateway timeout handling과 negative eval DB error handling을 보강했다.
- (Session 42 — 2026-07-05) **Ollama Qwen2.5-Coder 7B full eval**: `qwen2.5-coder:7b`를 gateway `text2sql_gateway_ollama_v1`로 연결해 24 positive + 14 negative eval을 실행했다. 첫 positive run은 모델 SQL의 `CardinalityViolation`으로 evaluator가 중단되어, DB execution error를 `FAIL`로 기록하고 계속 진행하도록 `run_text2sql_v2_eval.py`를 보강했다. 최종 positive 결과는 `8 PASS / 11 FAIL / 5 REFUSED / 0 BLOCKED`, score `52.53`; negative/content-safety는 `14/14 PASS`.
- (Session 41 — 2026-07-05) **Text2SQL content-safety negative eval**: 욕설/모욕, 성적 콘텐츠, 폭력/협박 콘텐츠 입력에 대한 negative 문항 6개를 추가해 전체 negative set을 14문항으로 확장했다. `run_text2sql_negative_eval.py`는 이제 거절/차단 여부뿐 아니라 `forbidden_output_terms` echo를 `FAIL_UNSAFE_ECHO`로 잡는다. 검증은 negative `14/14 PASS`, `negative_pass_rate=1.0`, 전체 `pytest 35 passed`.
- (Session 40 — 2026-07-05) **Text2SQL eval set hardening + chart**: positive expected-SQL 문항을 18개에서 24개로 확장하고, 큰 결과셋 문항에는 `limit`, 애매한 표현에는 `Top N`을 명시했다. 전혀 무관한 질문, 위험 SQL, 민감정보, 애매한 질문을 다루는 negative set 8개와 runner를 추가했다. 검증은 expected SQL `24/24 PASS`, v2 mock `13 PASS / 11 REFUSED / 0 BLOCKED`, negative `8/8 PASS`, `negative_pass_rate=1.0`. Eval chart는 `docs/images/06_text2sql_eval_summary.svg`.
- (Session 39 — 2026-07-05) **Local Text2SQL model evaluation rubric**: Ollama `qwen2.5-coder:7b`와 다른 local model을 같은 기준으로 비교하기 위해 Spider/BIRD 관행을 조사하고, AdInsight용 scoring model을 추가했다. `run_text2sql_v2_eval.py` summary는 이제 `model_score`를 포함하며, 점수는 execution accuracy, total pass coverage, safety, p95 latency를 합산한다. 문서: `docs/analysis/stage6_text2sql_local_model_eval_rubric.md`.
- (Session 38 — 2026-07-05) **Ollama-backed gateway smoke**: Ollama `qwen2.5-coder:7b`를 local model backend로 붙여 `/query/v2 -> gateway -> Ollama -> validator -> Postgres` 경로를 검증했다. 첫 실행은 schema context 부족으로 `UndefinedColumn`이 났고, `SCHEMA_CONTEXT_V1`에 실제 컬럼과 canonical query example을 추가한 뒤 `/query/v2`가 `200 OK`, rows `5`, top campaign `camp_000029`, latency `4800.432ms`를 반환했다.
- (Session 37 — 2026-07-05) **Local model gateway backend**: 외부 LLM API 대신 로컬 small model을 쓸 수 있도록 `TEXT2SQL_GATEWAY_BACKEND=ollama` backend를 추가했다. Gateway는 Ollama-compatible `/api/generate`를 호출하고, 모델 응답을 Text2SQL JSON contract로 파싱한다. Invalid JSON이나 contract mismatch는 SQL 실행으로 넘기지 않고 `not_answerable`로 처리한다. 테스트는 `27 passed`.
- (Session 36 — 2026-07-05) **Gateway-through-API smoke**: gateway `8010`과 serving API `8000`을 로컬에서 동시에 띄워 `/query/v2 -> Text2SQL gateway -> SQL validator -> Postgres` 경로를 검증했다. `/query/v2`는 mode `llm_generated_sql_v2_http_json`, rows `5`, top campaign `camp_000029`, latency `58.981ms`를 반환했다.
- (Session 35 — 2026-07-05) **Text2SQL gateway skeleton**: 웹 UI까지 고려해 LLM provider 호출 책임을 serving API에서 분리하는 gateway-first 구조를 채택했다. `text2sql_gateway/main.py`에 `/health`, `/text2sql/generate`를 추가했고, 현재는 mock backend로 동일 JSON contract를 반환한다. Docker Compose에 `text2sql-gateway` 서비스(port `8010`)를 추가하고, `docs/architecture/text2sql_gateway_architecture.md`에 Web UI -> Serving API -> Gateway -> Provider -> Postgres 흐름을 문서화했다.
- (Session 34 — 2026-07-05) **Resume bullets**: AdInsight의 data platform, ML/ROAS prediction, Text2SQL, DevOps/quality, AWS mapping 성과를 이력서 bullet 형식으로 정리한 `docs/resume_bullets.md`를 추가했다.
- (Session 33 — 2026-07-05) **Architecture diagram export**: 기존 Mermaid architecture note를 README용 versioned SVG asset `docs/images/00_architecture.svg`로 전환했다. 다이어그램은 ingestion, dbt modeling, ai_native/ROAS/Text2SQL eval, FastAPI/Superset/CI/AWS target mapping을 한 화면에 묶는다.
- (Session 32 — 2026-07-05) **Portfolio API examples + demo assets**: `/query/v2` request/response examples, 3-5분 demo script, 면접 talking points를 추가했다. README와 `docs/portfolio_draft.md`에서 새 자산을 링크하고, Text2SQL failure case와 hallucination 방지 설명을 면접용 증거 위치로 연결했다.
- (Session 31 — 2026-07-05) **Text2SQL v2 focused coverage expansion**: `MockSqlGenerationClient`에 LIMIT가 명확한 creator-review 질문 5개를 추가해 v2 mock eval을 `13 PASS / 5 REFUSED / 0 BLOCKED`로 개선했다. 중간 검증에서 no-LIMIT creator list가 validator에 의해 `BLOCKED`되는 것을 확인했고, 해당 케이스는 mock answerable에서 제외해 안전 기준을 유지했다.

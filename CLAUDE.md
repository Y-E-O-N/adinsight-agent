...
## 10. 현재 Phase
**Phase 5/6 준비 — campaign ROAS prediction monitoring 완료 후 Text2SQL/ML/API 확장 대기**
...
- 현재 구현 우선순위:
  ...
  5. **다음 후보**: Text2SQL campaign ROI eval 확장 → ML model v1 → FastAPI serving skeleton → AWS target architecture/IaC skeleton.
...
- 최신 검증: full `dbt run` `15/15 PASS`, full `dbt test` `248/248 PASS`, `campaign_roas_prediction_daily` Airflow manual run success.
...
- 다음: campaign ROI/prediction monitor를 Text2SQL 평가셋에 추가하거나, sklearn/LightGBM style ROAS model v1을 baseline과 비교한다.

...
## 10. 현재 Phase
**Phase 5/6 준비 — Text2SQL campaign ROI eval 확장 완료 후 ML/API 확장 대기**
...
- 현재 구현 우선순위:
  ...
  6. **다음 후보**: ML model v1 → FastAPI serving skeleton → AWS target architecture/IaC skeleton.
...
- 최신 검증: full `dbt run` `15/15 PASS`, full `dbt test` `248/248 PASS`, `campaign_roas_prediction_daily` Airflow manual/scheduled run success, expected-SQL evaluator `18/18 PASS`.
...
- 다음: sklearn/LightGBM style ROAS model v1을 baseline과 비교하거나, FastAPI `/predict` skeleton으로 model serving API를 시작한다.

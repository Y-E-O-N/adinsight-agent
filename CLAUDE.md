...
- Text2SQL gateway 최신 결과: `text2sql_gateway/main.py`를 추가해 별도 FastAPI gateway `/text2sql/generate`를 제공한다. 현재 backend는 `MockSqlGenerationClient`이고, 선택적으로 `TEXT2SQL_GATEWAY_API_KEY` bearer 인증을 요구한다. Docker Compose에는 `text2sql-gateway` 서비스(port `8010`)를 추가했다. Unit tests 포함 후 `uv run pytest -q`는 `24 passed`.

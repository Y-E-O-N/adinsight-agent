...
## 10. 현재 Phase
**Phase 6 준비 — ROAS model comparison + FastAPI artifact-backed serving 완료 후 API hardening/AWS 확장 대기**
...
- 현재 구현 우선순위:
  ...
  8. **다음 후보**: API latency/pytest smoke tests → README API usage → AWS target architecture/IaC skeleton.
...
- 최신 검증: full `dbt run` `15/15 PASS`, full `dbt test` `248/248 PASS`, `campaign_roas_prediction_daily` Airflow manual/scheduled run success, expected-SQL evaluator `18/18 PASS`, FastAPI `/health` and `/predict/campaign-roas` smoke `200 OK`.
...
- Push 상태: 기능 코드 기준 최신 커밋은 `371f24d Load ROAS model artifact in FastAPI`이고, 이후 handoff 문서 커밋이 이어진다.
- 남은 로컬 변경: `dags/dag_ig_collect_daily.py`에서 candidate hashtags 대부분이 주석 처리되어 있다. 이는 API/ML 작업과 별도 변경이므로 새 세션에서 의도 확인 후 커밋 또는 복원한다.
- 다음: `dags/dag_ig_collect_daily.py` 변경 의도 확인 → API latency/pytest smoke tests 또는 AWS target architecture/IaC skeleton.

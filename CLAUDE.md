...
- External LLM gateway backend 최신 결과: `TEXT2SQL_GATEWAY_BACKEND=openai|gemini`을 추가했다. OpenAI backend는 JSON Schema structured output contract를 사용하고, Gemini backend는 `application/json` schema response format을 사용한다. `TEXT2SQL_EVAL_MODEL_LABEL`을 추가해 OpenAI/Gemini 모델명도 eval metrics의 `local_model` 필드에 기록할 수 있게 했다. Unit tests는 mocked HTTP response 기준으로 OpenAI/Gemini payload와 parsing을 검증한다. 현재 로컬 `.env`에는 OpenAI/Gemini API key가 없어 real external eval은 pending이다.

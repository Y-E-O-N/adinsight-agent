...
- Local Text2SQL model evaluation 최신 결과: `docs/analysis/stage6_text2sql_local_model_eval_rubric.md`에 Spider/BIRD 관행 기반 평가 기준을 정리했고, `agent/eval/text2sql_model_scoring.py`를 추가해 `run_text2sql_v2_eval.py` summary에 0~100점 `model_score`와 tier를 포함하도록 했다. 기준은 answerable execution 45%, total pass coverage 25%, safety 20%, p95 latency 10%이다. Negative/refusal set은 `agent/eval/text2sql_negative_questions.yml`, runner는 `agent/eval/run_text2sql_negative_eval.py`이다.

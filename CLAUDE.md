...
- Local Text2SQL model evaluation 최신 결과: `docs/analysis/stage6_text2sql_local_model_eval_rubric.md`에 Spider/BIRD 관행 기반 평가 기준을 정리했고, `agent/eval/text2sql_model_scoring.py`를 추가해 `run_text2sql_v2_eval.py` summary에 0~100점 `model_score`와 tier를 포함하도록 했다. 기준은 answerable execution 45%, total pass coverage 25%, safety 20%, p95 latency 10%이다.
- Text2SQL v2 quality 최신 결과: LIMIT가 명확한 creator-review mock coverage를 추가해 live eval 기준 `13 PASS / 5 REFUSED / 0 BLOCKED`, answerable exec_acc `1.0`, refuse_rate `0.2778`이다. No-LIMIT broad list는 `docs/analysis/stage6_text2sql_v2_failure_cases.md`에 failure/refusal case로 남겼다.
